document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!Number.isFinite(credits) || credits <= 0) credits = START_CREDITS;
  ...
  // Ultimate X award table by hand category
  const AWARD = {
    nothing: 1,
    jacks_or_better: 2,
    two_pair: 2,
  ...
  const modeEl = document.getElementById("mode"); // reserved for policy later
  const btn = document.getElementById("dealDrawBtn");

  /* ===============================
     STATE
     =============================== */
  let phase = "choosing"; // choosing (bottom shown) → resolved (all shown)
  let baseHand = [];
  ...
  /* ===============================
     EXACT DDB EVALUATOR (category + payout)
     =============================== */
  function evaluateDDB(cards, pt){
    ...
    if(isFlush && isStraight){
      if(ranks.includes("A") && ranks.includes("T")) return { category:"royal_flush", payout:800 };
    ...
    if(counts[0]===3 && counts[1]===2) return { category:"full_house", payout:pt.full_house };
    ...
    if(counts[0]===2){
      ...
      if(["J","Q","K","A"].includes(pair)) return { category:"jacks_or_better", payout:1 };
    }
  ...
  function renderFaceup(box, cards, mask, clickable){
    ...
    for(let i=0;i<5;i++){
      ...
      if(clickable){
        el.onclick = () => {
          mask[i] = !mask[i];
          renderChoosing(); // rerender bottom only
        };
      } else {
  ...
  /* ===============================
     EV (immediate + 1-step lookahead)
     =============================== */
  function estimateEV(mask, samples=700){
  ...
  /* ===============================
     MACHINE FLOW (single Deal/Draw button)
     =============================== */
  ...
  // Deduct credits when a new hand is presented (machine feel)
  function chargeForHand(){
    const cost = COST_PER_HAND * NUM_HANDS;
    if(credits < cost){
      // auto-refill so you can always practice
  ...
  btn.onclick = () => {
    if (phase === "choosing") {
      ...
      const roundWin = out.winTop + out.winMid + out.winBot;
      credits += roundWin;
      ...
      resultBox.innerHTML = `
        <b>Win:</b> +${roundWin.toFixed(0)} credits<br><br>
        Top: ${out.topRes.category} → next ×${out.nextMult.top}<br>
        Mid: ${out.midRes.category} → next ×${out.nextMult.mid}<br>
        Bot: ${out.botRes.category} → next ×${out.nextMult.bot}<br><br>
        <b>EV loss:</b> ${loss}
      `;

      updateUI();

      // Chain multipliers automatically for the next hand
      currentMult = { ...earnedNext };

      // Immediately start next hand (machine feel)
      chargeForHand();
      startNewHand();
      return;
    }
  ...
  // Persist selection
  paytableEl.value = localStorage.getItem("ux_pt") || "DDB_9_6";
  paytableEl.onchange = () => localStorage.setItem("ux_pt", paytableEl.value);
  modeEl.value = localStorage.getItem("ux_mode") || "conservative";
  modeEl.onchange = () => localStorage.setItem("ux_mode", modeEl.value);
